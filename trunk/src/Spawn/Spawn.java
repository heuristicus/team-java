...
public class Spawn implements Serializable {
    ...
    public Unit newBossUnit(Point spPoint) {
        Unit boss = new Enemy(10000, 1, new UltraDeathWeapon(), new SuperSlowPath(), 100, spPoint.x, spPoint.y, Color.cyan);
        return boss;
    }
        ...
        for (int i = 0; i < 8; i++) {
            if (i == spawnPoint) {
                if (Math.random() < 0.008) {
                    arr.add(newBossUnit(spawnPoints[i]));
                } else {
